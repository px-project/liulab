...
import { Upload, Loader, FormGroup, Map } from '../../../common/';
import './style.scss';
...
export class CategoryEditComponent extends Component {
	...
	render() {
		...
		return (
			<Loader className="category-edit-page page" loading={page === 'edit'}>
				...
				<div className="field-config sec ui form">
					...
					<ol className="list">
						{formData.attrs ? formData.attrs.map((item, index) => (
							<li key={index}>
								<div>
									<Map className="field-title" title="字段名">{item.title}</Map>
									<Map className="field-required" title="必填">{item.attr_required ? '是' : '否'}</Map>
								</div>
								<div>
									<Map className="field-key" title="数据名">{item.key}</Map>
									<Map className="field-type" title="类型">{item.attr_type}</Map>
								</div>
							</li>
						)) : ''}
					</ol>
